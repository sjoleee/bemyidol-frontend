...
const PositionSettingCard = ({ ...props }: extendedProps) => {
  ...
  return (
    <div
      className={`flex w-full md:w-[calc(50%-4px)] h-24 rounded-lg px-4 py-2 items-center ${
        props.idx % 2 ? 'bg-white' : 'bg-PRIMARY_LIGHT'
      } ${props.idx % 4 === 1 || props.idx % 4 === 2 ? 'md:bg-white' : 'md:bg-PRIMARY_LIGHT '}`}
    >
      <div className="flex w-[68px] h-[68px] mr-4">
        <img
      ...
      <div className="flex flex-col w-[calc(100%-140px)]">
        <T5>{props.groupName}</T5>
        <T2>{props.name}</T2>
        <PositionSelect memberId={props.memberId} idx={props.idx} position={props.position} />
      </div>
      <label className="flex items-center justify-end ml-4 w-[40px] whitespace-nowrap ">
